...
import com.biklom.wiki.objects.Dungeon;
...
public class DataExtractor {
    ...
    @Autowired
    private WikiUpdateConfig config;
    ...
    public void generateData() {
        ...
        try (Workbook workbook = WorkbookFactory.create(file)) {
            ...
            dumpSkillsToFile(namedSortedSkills, SKILL_TYPE.BASIC,true);
            dumpSkillsToFile(namedSortedSkills, SKILL_TYPE.LEADER,true);
            dumpSkillsToFile(namedSortedSkills, SKILL_TYPE.SPECIAL,true);
            dumpVisData(mapUnitByCode);
            
            try(WikiUpdater wu = new WikiUpdater(config)) {
                wu.login()
                    .init()
                    .createMissingUnit(mapUnitByCode.values())
                    .createMissingUnitAliases(mapUnitByCode.values())
                    .createMissingSkills(namedSortedSkills)
                    .createMissingSkillsAliases(namedSortedSkills);
            }
    ...
    private void dumpVisData(Map<String, Unit> mapUnitByCode){
        StringBuilder nodes = new StringBuilder("  var nodes = new vis.DataSet([\n");
        StringBuilder edges = new StringBuilder();
        HashSet<Integer> ids = new HashSet<>();
        mapUnitByCode.values().forEach(u -> {
            int id = Integer.valueOf(u.getBestiaryslot());
            if(!ids.contains(id)) {
                ids.add(id);
                nodes.append("{id:").append(id).append(",shape:'image',image:'./images/Monsters/Cards/unit_").append(u.getCodename()).append("_card.png',group:").append(id).append("},\n");
                if(StringUtils.isNotBlank(u.getMorphsinto())) {
                    edges.append("{from:").append(id).append(", to:").append(Integer.valueOf(mapUnitByCode.get(u.getMorphsinto()).getBestiaryslot())).append(", dashes:false,width:3, arrows: {to: true }},\n");
                }
                if(StringUtils.isNotBlank(u.getMaterial1()) && mapUnitByCode.get(u.getMaterial1().replaceAll("(.*)/(.*)", "$2")) != null) {
                    edges.append("{from:").append(Integer.valueOf(mapUnitByCode.get(u.getMaterial1().replaceAll("(.*)/(.*)", "$2")).getBestiaryslot())).append(", to:").append(id).append(", dashes:true,width:1, arrows: {to: true }},\n");
                }
                if(StringUtils.isNotBlank(u.getMaterial2()) && mapUnitByCode.get(u.getMaterial2().replaceAll("(.*)/(.*)", "$2")) != null) {
                    edges.append("{from:").append(Integer.valueOf(mapUnitByCode.get(u.getMaterial2().replaceAll("(.*)/(.*)", "$2")).getBestiaryslot())).append(", to:").append(id).append(", dashes:true,width:1, arrows: {to: true }},\n");
                }
                if(StringUtils.isNotBlank(u.getMaterial3()) && mapUnitByCode.get(u.getMaterial3().replaceAll("(.*)/(.*)", "$2")) != null) {
                    edges.append("{from:").append(Integer.valueOf(mapUnitByCode.get(u.getMaterial3().replaceAll("(.*)/(.*)", "$2")).getBestiaryslot())).append(", to:").append(id).append(", dashes:true,width:1, arrows: {to: true }},\n");
                }
            }
        });
        nodes.append("  ]);\n" +
...
"  var edges = new vis.DataSet([\n").append(edges).append("  ]);\n");
        try {
            FileUtils.writeStringToFile(new File(outputDir, "vis.js"), nodes.toString(), "UTF-8");
        } catch (IOException ex) {
            LOGGER.error( null, ex);
        }
    }
    private void dumpSkillsToFile(Collection< Skill> skills, SKILL_TYPE category,boolean dumpUnused) {
        StringBuilder sb = new StringBuilder();
        sb.append("return {\n");
        skills.stream().forEach((u) -> {
            if(SKILL_TYPE.findType(u.getInternalCode()) == category) {
                if (u.isUsed() || dumpUnused) {
                    sb.append(u.toString());
                } else {
